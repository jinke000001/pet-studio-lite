...
function recordGate(root, gate, { evidenceRoot = root } = {}) {
  ...
  const latestGates = run.state.gates.filter((entry) => entry.id !== attempt.id).concat(attempt);
  const next = { ...run.state, gates: latestGates, attempts: [...run.state.attempts, attempt], evidenceFiles: [...new Set([...run.state.evidenceFiles, ...attempt.evidencePaths])], firstFailedGate: attempt.status === 'failed' ? (run.state.firstFailedGate || attempt.id) : (run.state.firstFailedGate || null), updatedAt: new Date().toISOString() }; atomicWriteJson(path.join(run.root, 'acceptance-state.json'), next); return attempt;
}
