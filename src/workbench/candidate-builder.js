...
function createStudioBuilderConfiguration({ outputDirectory, version = '0.1.0' }) {
  return {
    ...
    files: ['package.json', 'src/**/*', 'config/**/*', 'build/**/*', '.workbench-dist/**/*', '!**/.DS_Store', '!**/._*'],
...
function buildStudioCandidate({ projectRoot, targets = ['mac'], now = new Date(), spawn = childProcess.spawnSync }) {
  ...
  const config = createStudioBuilderConfiguration({ outputDirectory: runDirectory });
