...
- [x] 4.2 准备版本化 v2 制作运行目录。
- [ ] 4.3 生成并由用户确认小福猩主视觉基准。（候选已生成，等待用户视觉确认）
- [ ] 4.4 制作、逐行验证并确认 9 种标准动作。
- [ ] 4.5 制作、盲测并确认 4 个基准方向和 16 个注视方向。
- [ ] 4.6 组装 `1536×2288` v2 图集、标准包和完整 QA 证据。
- [ ] 4.7 通过通用运行器做 source-mode 与 macOS 真实窗口验证。
- [ ] 4.8 通过 Phase 3 工厂生成小福猩双平台未签名候选。
