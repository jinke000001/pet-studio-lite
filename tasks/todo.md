...
- [ ] 6.1a 定义项目、步骤、任务、错误和产物契约。
- [ ] 6.1b 建立版本化非覆盖项目存储，并验证创建、列出、打开和关闭后恢复。
- [ ] 6.1c 新增 React + TypeScript 工作台壳层及独立、安全的 Electron 入口与最小 IPC。
- [ ] 6.1d 生成 Kimi 页面/字段/状态/错误/模拟数据交接包。
- [ ] 6.1e 通过新增测试、原 77 项回归、lint、前端构建和 macOS 真实启动检查。
- [ ] Checkpoint 6.1 用户审查工作台基础后，再进入 Phase 6.2。
