...
- [ ] C5 Windows 真实 RETURN：使用当前正式交接 `phase-6-workbench-windows-recheck-20260907-04` 在本地 NTFS 新目录从头执行；source → win-unpacked → installed mode，再执行 DPI、安装/卸载/重装和用户确认；当前仍不得宣称 Windows 验收通过。
...
- [x] C10 官方卸载复验修复：提交 `fcd1085`，固定 NSIS GUID/显示名，新增只调用 HKCU 官方 `UninstallString` 的探针，并以卸载退出码、32/64 位注册项、安装目录、快捷方式和精确进程连续 3 次清零为通过条件；Windows 候选与正式本地交接更新为 `candidate-20260907150456543`、`20260907-04`。误生成的 `20260907-03` 仅保留为非正式清单尝试。
...
- [x] 6.6k `-09` Windows RETURN 复核与根因修复：回传（`RETURN/20260902-172428/`）哈希 68/68 通过、验证器仅 `overallStatus=failed` 预期失败，首个失败门 `G5-04-candidate-export-source`。确诊并修复四层缺陷：候选构建 argv 被 yargs hideBin 误裁（`25f5779`，仓库内 builder-launcher 规范化）、builder 子进程写 default_app.asar 被 ASAR 补丁拦截（`7effcfa`，launcher 内 `process.noAsar`）、候选 files 映射丢 package.json（`8d696ad`，目录+filter 形式）、工作台进程读候选 app.asar 被补丁拦截（`e4eb38c`/`fe6905b`，检查器与哈希局部关补丁）；同时修复 packaged 预览 app.asar cwd（`25f5779`）与 RETURN validator-output 自引用流程（验证器新增拒绝 RETURN 内路径的 `--output`）。聚焦 117/117、全量 222/222、全部质量门通过；macOS 候选 `candidate-20260902153954972`（绑定 `fe6905b`）packaged smoke 全项通过（启动/单实例/预览启停/应用内 macOS 候选构建成功/进程清零，证据在候选 `smoke/` 目录）；Windows x64 交叉候选 `candidate-20260902161102333` 已生成（仅静态参照）。
- [x] 6.6l `-10` Windows 全量复验：主回传 75/75 哈希通过，但补验回传（`RETURN/20260903-194712-supplement/`，15667/15667 哈希通过）overallStatus=failed，首个失败门 `S6-defect-preview-killed-by-job-ack-loop`（打开含历史成功任务的项目后立即启动预览被任务刷新静默停止）；S7 未执行、S8 待用户确认。本门以失败收口，由 6.6m 取代。
- [x] 6.6m-1 预览刷新缺陷修复（提交 `e5a69ba2e0573a8cdb362d6ec6bc868c364e2d66`）：新增 `src/workbench/project-lifecycle.js`（集中跟踪 activeProjectId；同项目重开=只读重载，仅切换项目才取消旧任务/停止它项目预览；get-bootstrap/create-project 登记活动项目身份）；renderer 轮询改用 `src/workbench/renderer/job-refresh.js` 观察器（首次轮询只建基线，历史成功任务不再触发项目打开；仅迁移到 succeeded 的任务触发刷新）。新增 12 项行为回归，其中 3 项对旧语义确定性失败；旧代码 `c0ce641` worktree 端到端复现预览约 3 秒被杀。聚焦 129/129、全量 234/234、typecheck/build/lint/preflight/audit/diff-check 全部通过。
...
- [x] 6.6p 官方卸载复验修复（2026-09-07）：全量 246/246 与全部质量门通过；Windows x64 候选绑定 `fcd1085`，NSIS SHA-256 `06be0bd9091ece90a3dd0a554d2812962816e64a9705f1f8cd2dc3234756374f`；正式交接 `release/handoff/phase-6-workbench-windows-recheck-20260907-04/` 的 33/33 checksums、bundle、60 门身份、夹具和空 RETURN 均验证通过，未写 T7。Windows 60/60 实机 RETURN 仍待外部完成。
