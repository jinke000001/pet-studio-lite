...
- 0.1.0 Windows 回传已检查：输入与回传哈希通过，但 source 路径兼容/lint/Electron 二进制门、150% 冷启动、动态 DPI 和官方卸载失败。0.1.1 已完成针对性修复、显式 Electron 安装、75/75 回归（连续 5 轮通过）、批准素材哈希保护、macOS 真实运行和双平台候选静态验证；新 T7 复验包尚未写入，等待用户确认目标。
