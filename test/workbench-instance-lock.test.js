...
test('throws instead of returning null when dead-lock recovery is persistently contested', () => {
  const directory = makeTempDir();
  try {
    fs.writeFileSync(lockPathOf(directory), JSON.stringify({ pid: 999999, token: 'contested' }));
    // 模拟对抗者：每次我们删除死锁后、发布前，锁又被放回（通过删除时立即重建模拟）。
    const originalUnlinkSync = fs.unlinkSync;
    fs.unlinkSync = (target, ...rest) => {
      originalUnlinkSync(target, ...rest);
      if (target === lockPathOf(directory)) {
        fs.writeFileSync(lockPathOf(directory), JSON.stringify({ pid: 999999, token: 'contested' }));
      }
    };
    try {
...
// 审查发现 9：死亡锁删除遇到瞬时 EBUSY/EPERM 时在有限重试内恢复，最终仍失败则原样抛出。
test('retries transient EBUSY when removing a dead lock and rethrows if persistent', () => {
  const directory = makeTempDir();
  try {
    fs.writeFileSync(lockPathOf(directory), JSON.stringify({ pid: 999999, token: 'stuck' }));
    const originalUnlinkSync = fs.unlinkSync;
    fs.unlinkSync = (target, ...rest) => {
      if (target === lockPathOf(directory)) throw Object.assign(new Error('EBUSY'), { code: 'EBUSY' });
...
test('publication never opens the lock path; record reads use an explicit no-follow descriptor', () => {
  ...
  try {
    // (a) 悬空链接：发布只经 linkSync；对锁路径的 openSync 一次都不允许。
    fs.symlinkSync(target, lockPath);
    assert.throws(() => acquireInstanceLock({ directory, pid: 200 }), /实例锁不安全/);
    assert.equal(fs.existsSync(target), false);
    ...
    assert.equal(linkCalls.some((call) => call.dest === lockPath), true,
      'publication must attempt linkSync onto the lock path');
    fs.unlinkSync(lockPath);
    openCalls.length = 0;
    linkCalls.length = 0;
    // (b) 合法活锁：读取记录必须显式打开锁路径，且在支持平台上带 O_NOFOLLOW。
    const holder = acquireInstanceLock({ directory, pid: 100, signal: liveSignal });
    openCalls.length = 0;
    assert.equal(acquireInstanceLock({ directory, pid: 200, signal: liveSignal }), null);
...
test('exhaustion after contested recovery reports unsafe, not a stale transient error', () => {
  ...
  try {
    ...
    fs.unlinkSync = (targetPath, ...rest) => {
      if (targetPath === lockPathOf(directory)) {
        if (first) {
          first = false;
          throw Object.assign(new Error('EPERM'), { code: 'EPERM' });
        }
        originalUnlinkSync(targetPath, ...rest);
        plant(); // 对抗者立即回填死锁
        return;
      }
      return originalUnlinkSync(targetPath, ...rest);
    };
    try {
      assert.throws(
        () => acquireInstanceLock({ directory, pid: 200, signal: deadSignal }),
