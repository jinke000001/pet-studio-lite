...
- 实现提交：`37738c3bd557fb580a9c78a284ed68183b493163`
- 自动测试：73/73 通过；lint 通过；`npm audit --omit=optional` 为 0 vulnerabilities；`git diff --check` 通过。
...
- 先核对复验包全量哈希，再执行 `npm ci`、Electron 二进制预检、73/73 测试和 lint；source 门失败即停止后续额度消耗。
