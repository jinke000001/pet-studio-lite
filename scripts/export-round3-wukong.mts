// 生成返修轮 Windows x64 内部测试 ZIP：直接使用「没有 license 的真实 Petdex
// Wukong WebP 包」（~/.petdex/pets/wukong-6），不修改 pet.json 一个字节 ——
// 导入时自动成为 usageMode=internal-test，产物 distribution=internal-test-only。
...
async function main(): Promise<void> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'pet-export3-'));
