// 生成第六轮 Windows x64 候选 ZIP（缩放档位简化轮：全产品统一三档缩放
// 小 100% / 中 150%（默认）/ 大 200%；旧档 50%/75% 在读取时自动提升为 100%，
// 非法/越界值安全回落；右键菜单与启动说明同步去掉 50%/75%）。
...
async function main(): Promise<void> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'pet-export6-'));
