// 生成第四轮 Windows x64 候选 ZIP（体验优化轮：右键"自动游走"勾选开关 +
// "回到屏幕右下角"、气泡均衡换行、导出后"打开所在文件夹"受控校验）。
// 与第三轮相同：直接使用「没有 license 的真实 Petdex Wukong WebP 包」
...
async function main(): Promise<void> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'pet-export4-'));
